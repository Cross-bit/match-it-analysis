...
class ItemKnnCFEvaluation(SurpriseRatingBasedEvaluation):

    def __init__(self, rating_matrix, k, algorithm_k, test_size=0.2, rating_scale=(1, 5)):
        self.k = k
        self.algorithm_k = algorithm_k
        super().__init__(rating_matrix, test_size, rating_scale)

    def fit(self):
        model = ItemKnnCFModel(self.trainset, self.algorithm_k)
        self.algo = model
