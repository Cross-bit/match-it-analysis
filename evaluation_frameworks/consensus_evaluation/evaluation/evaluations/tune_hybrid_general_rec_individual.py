...
class TuneHybridGeneralRecIndividual(ConsensusExperimentBase):
    """Stejný zásobník jako ``eval_hybrid_general_rec_individual``; mřížka hlavně ``first_r_ration``."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if sigmoid_params is not None:
            self.sigmoid_params = copy.deepcopy(sigmoid_params)
        else:
            self.sigmoid_params = copy.deepcopy(dict(type(self).DEFAULT_SIGMOID_PARAMS))
            if fast_grid:
                for d in self.sigmoid_params.values():
                    fr = list(d["first_r_ration"])
                    if len(fr) > 2:
                        d["first_r_ration"] = [fr[0], fr[-1]]
...
if __name__ == "__main__":
    autorun(TuneHybridGeneralRecIndividual)
