...
class TuneAsyncStaticPolicySimplePriorityGroupRec(ConsensusExperimentBase):
    ...
    def _t_values_static_threshold(self) -> List[int]:
        # range(1, w) is empty for w<=1; eval default for W=1 is t=0
        if self.w_size <= 1:
            return [0]
        return list(range(1, self.w_size))
...
if __name__ == "__main__":
    autorun(TuneAsyncStaticPolicySimplePriorityGroupRec)
